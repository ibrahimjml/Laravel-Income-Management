...
<div class="d-flex flex-wrap gap-3 justify-content-start">
  @forelse($today_payments as $today)
    <div class="card shadow-sm border-0 flex-grow-1" style="min-width: 320px; max-width: 420px;">
      <div class="card-body d-flex gap-4"><!-- start card body -->
        ...
        <div class="mb-3 w-100"><!-- start payment card -->
         <!-- client full name -->
          <a href="{{ route('details',['income'=>$today->income_id]) }}" style="text-decoration: none;">
          <h5 class=" mb-2 text-primary border-primary border-bottom pb-2">{{ $today->client->full_name }}</h5>
                ...
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <div>
                          <span class="badge bg-{{ 
                            $today->status == \App\Enums\IncomeStatus::COMPLETE ? 'success' : 
                            ($today->status == \App\Enums\IncomeStatus::PARTIAL ? 'warning' : 'danger') 
                        }}">
                            {{ $today->status->label() }}
                        </span>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <div>{{ date('M d, Y', strtotime($today->next_payment)) }}
                        @if($today->next_payment)
                       <span class="border-start border-dark border-2 ps-1">
                           @php
                               $paymentDate = $today->next_payment->startOfDay();
