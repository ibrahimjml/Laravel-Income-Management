          ...
          <form id="editPaymentForm" method="POST">
            ...
            <input type="hidden" id="edit_payment_id" name="payment_id">
              <div class="modal-header">
                  <h5 class="modal-title" id="editPaymentModalLabel">{{ __('message.Edit Payment') }}</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                  <div class="mb-3">
                      <label for="edit_amount" class="form-label">{{ __('income.Payment Amount') }}</label>
                      <input type="number" class="form-control border" id="edit_amount" name="payment_amount"
                          max="{{$income->remaining}}" required>
                  </div>
                  <div class="mb-3">
                      <label for="edit_status" class="form-label">{{ __('message.Status') }}</label>
                  ...
                  <div class="mb-3">
                      <label for="edit_next_payment" class="form-label">{{ __('message.Next Payment Date') }}</label>
              ...
              <div class="modal-footer d-flex justify-content-between">
                  <button type="submit" class="btn btn-success">{{ __('message.Edit Payment') }}</button>
