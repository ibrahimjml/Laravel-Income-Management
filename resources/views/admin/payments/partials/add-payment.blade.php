          ...
          <form action="{{route('add.payment',$income->income_id)}}" id="addPaymentForm" method="POST">
            ...
            @method('POST')
              <div class="modal-header">
                  <h5 class="modal-title" id="addPaymentModalLabel">{{ __('message.Add Payment') }}</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                  <div class="mb-3">
                      <label for="payment_amount" class="form-label">{{ __('income.Payment Amount') }}</label>
                      <input type="number" class="form-control border" id="payment_amount" name="payment_amount"
                          max="{{$income->remaining}}" required>
                  </div>
                  <div class="mb-3">
                      <label for="payment_status" class="form-label">{{ __('message.Status') }}</label>
              ...
              <div class="modal-footer d-flex justify-content-between">
                  <button type="submit" class="btn btn-success">{{ __('message.Add Payment') }}</button>
